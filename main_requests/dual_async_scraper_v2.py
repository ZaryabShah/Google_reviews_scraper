...
class DualAsyncGoogleMapsReviewScraper:
    ...
    def parse_reviews_from_response(self, html_content, sort_direction):
        """Parse reviews from the HTML response with duplicate detection per request"""
        reviews = []
            ...
            for i in range(max_reviews):
                ...
                with self.lock:
                    ...
                    if review_id in self.seen_review_ids or reviewer_id in self.seen_reviewer_ids:
                        ...
                        # Check if THIS REQUEST has too many duplicates (instead of total)
                        if duplicates_in_request > 15:
                            print(f"[{sort_direction}] STOPPING: More than 10 duplicates found in this single request!")
